...
public class HW7TestDriver {
    ...
    private void listChildren(String graphName, String parentName) {
        Graph<String, Double> graph = graphs.get(graphName);
        List<Edge<Double, String>> edges = graph.getEdges(parentName);
        output.print("the children of " + parentName + " in " + graphName + " are:");
        int edgeSize = edges.size();
        if (edgeSize > 0) {
            edges.sort((Edge<Double, String> e1, Edge<Double, String> e2) -> {
            ...
            for (int i = 0; i < edgeSize; i++) {
                Edge<Double, String> edge = edges.get(i);
    ...
    private void loadGraph(String graph, String file) {
        Graph<String, Double> marvelGraph = MarvelPaths2.loadGraph("src/hw7/data/" + file);
    ...
    private void findPath(String graphName, String start, String end) {
        ...
        if (!nodes.contains(start) || !nodes.contains(end)) {
            ...
            List<Edge<Double, String>> path = MarvelPaths2.findPath(graph, start, end);
            ...
            if (path == null) {
                ...
                for (int i = 0; i < path.size() - 2; i++) {
                    Edge<Double, String> edge = path.get(i);
                    String child = edge.getChild();
                    output.println(last + " to " + child + " with weight " + String.format("%.3f", edge.getLabel()));
                    last = child;
                }
                output.println("total cost: " + String.format("%.3f", path.get(path.size() - 1).getLabel()));
            }
        }
    }
}
